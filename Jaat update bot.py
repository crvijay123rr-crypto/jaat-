...
async def run_forwarding_process(c, status_msg, uid, dest_chat_id, f_data):
    ...
    sleep_time = 4.0  # Safe interval between copies to avoid flood wait
    ...
    async for bot_doc in user_bots_collection.find():
        ...
        try:
            ...
            async def child_steps_handler(c, msg):
                ...
                text = msg.text.strip()
               # await auto_delete(msg) # Retains links/texts securely as updated previously
            ...
            running_minibots[uid] = child_client
        except Exception:
            pass

# --- MASTER BOT ADD-BOT HANDLER ---
@app.on_message(filters.forwarded & filters.private)
async def addbot_handler(client, message):
    if message.reply_to_message:
        text = message.text or message.caption
        if text and (":" in text or "bot" in text.lower()):
            token_match = re.search(r'\d+:[A-Za-z0-9_-]+', text)
            if token_match:
                token = token_match.group(0)
                user_id = message.from_user.id
                status_msg = await message.reply_text("🔄 <b>Child-Bot Register ho raha hai, Kripya intezaar karein...</b>", parse_mode=ParseMode.HTML)
                ...
                try:
                    ...
                    # Copy-paste Child Handlers (Start, setchannel, forward, steps_handler, stop, run_fwd)
                    @child_client.on_message(filters.command("start"))
                    async def child_start(c, msg):
                        uid = msg.from_user.id
                        dest_stat = "❌ Not Set"
                    ...
                    async def child_setchannel(c, msg):
                        ...
                        if reply_msg:
                            if reply_msg.forward_from_chat:
                                ...
                                text_in = (reply_msg.text or reply_msg.caption).strip()
                                if text_in.startswith("-100") or text_in.startswith("-"):
                                    try: dest_chat_id = int(text_in)
                                    except: pass
                                else:
                                    chat_id, _ = await get_details_from_link(c, text_in)
                                    dest_chat_id = chat_id
                        else:
                            cmd_args = msg.text.split(" ", 1)
                            if len(cmd_args) > 1:
                                text_in = cmd_args[1].strip()
                                if text_in.startswith("-100") or text_in.startswith("-"):
                                    try: dest_chat_id = int(text_in)
                                    except: pass
                                else:
                    ...
                    async def child_steps_handler(c, msg):
                        ...
                        if step == "source":
                            chat_id, msg_id = await get_details_from_link(c, text_in)
                            if not chat_id:
                                await msg.reply_text("⚠️ <b>Invalid Source Link!</b> Kripya source channel ki valid post link bhejein.")
                                return
                            ...
                            if text_in.isdigit():
                                ...
                                _, msg_id = await get_details_from_link(c, text_in)
                            ...
                            c.forward_steps[uid]["data"]["caption"] = text_in if text_in.lower() != 'skip' else ""
                            await msg.reply_text("✅ <b>Setup Ready!</b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🚀 Start Forwarding", callback_data="run_fwd")]]))
                    ...
                    try:
                        await message.forward(chat_id=DATABASE_CHAT_ID)
                    ...
                    await auto_delete(message.reply_to_message)
...
# --- FORMATTED SUMMARY ENGINE (WITH GAPS & NUMBERING) ---
async def send_chunked_summary(client, chat_id, b_name, topic_list):
    ...
    for i in range(0, len(topic_list), chunk_size):
        ...
        # Gapped & Numbered Layout Integrated
        s_txt = f"✨ <b>{b_name} - Summary (Part {part_num}):</b>\n\n"
        for idx, (name, link) in enumerate(chunk, start=i+1):
            s_txt += f"<b>{idx}.</b> <a href='{link}'>{name}</a>\n\n"
